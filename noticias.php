    ...
    if ($action == 'add' || $action == 'edit') {
        ...
        if (empty($titulo) || empty($conteudo)) {
            ...
            if ($action == 'add') {
                ...
                $id = intval($_POST['id']);
                $stmt = $conn->prepare("UPDATE artigos SET titulo = ?, slug = ?, conteudo = ? WHERE id = ? AND categoria_id = 1");
        ...
        $id = intval($_POST['id']);
        $stmt = $conn->prepare("DELETE FROM artigos WHERE id = ? AND categoria_id = 1");
        ...
        if ($stmt->execute()) {
            ...
            $mensagem = "Erro ao excluir notícia: " . $stmt->error;
            $tipo_mensagem = 'error';
        }
        $stmt->close();
    }
...
$noticias = $conn->query("SELECT a.*, u.nome as autor_nome FROM artigos a LEFT JOIN usuarios u ON a.autor_id = u.id WHERE a.categoria_id = 1 ORDER BY a.criado_em DESC");
...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id'])) {
    $editando = true;
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("SELECT * FROM artigos WHERE id = ? AND categoria_id = 1");
    $stmt->bind_param("i", $id);
    $stmt->execute();
...
<div class="sidebar-footer">
        ...
        <main class="main-content">
            <header class="content-header">
                ...
                <p>Adicione, edite ou exclua notícias do site</p>
            </header>
            ...
            <?php if ($mostrar_form): ?>
                ...
                <div class="form-container">
                    <h2><?php echo $editando ? 'Editar Notícia' : 'Nova Notícia'; ?></h2>
                    <form method="POST" action="noticias.php">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="titulo" name="titulo" required 
                                   value="<?php echo $editando ? htmlspecialchars($noticia_edit['titulo']) : ''; ?>">
                        </div>
                        
                        <div class="form-group">
                            <label for="slug">Slug (URL amigável)</label>
                            <input type="text" id="slug" name="slug" 
                                   value="<?php echo $editando ? htmlspecialchars($noticia_edit['slug']) : ''; ?>"
                                   placeholder="Deixe em branco para gerar automaticamente">
                        </div>
                        
                        <div class="form-group">
                            <label for="conteudo">Conteúdo *</label>
                            <textarea id="conteudo" name="conteudo" required><?php echo $editando ? htmlspecialchars($noticia_edit['conteudo']) : ''; ?></textarea>
                        </div>
                        ...
                        <div class="form-actions">
                            ...
                            <a href="noticias.php" class="btn-secondary">Cancelar</a>
                        </div>
                    </form>
                </div>
            <?php else: ?>
                ...
                <div class="content-table">
                    <div class="table-header">
                        ...
                        <a href="noticias.php?action=add" class="btn-primary">+ Nova Notícia</a>
                    </div>
                            ...
                            <?php if ($noticias->num_rows > 0): ?>
                                <?php while($noticia = $noticias->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($noticia['autor_nome'] ?? 'N/A'); ?></td>
                                        <td><?php echo date('d/m/Y', strtotime($noticia['criado_em'])); ?></td>
                                        <td class="actions">
                                            <a href="noticias.php?action=edit&id=<?php echo $noticia['id']; ?>" class="btn-edit">Editar</a>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" style="text-align: center; padding: 40px;">
                                        Nenhuma notícia encontrada. <a href="noticias.php?action=add">Adicionar a primeira notícia</a>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
